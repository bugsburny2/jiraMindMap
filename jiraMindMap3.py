...
if not file_path:
    ...
    # Configuration dynamique des paramètres du graph
    node_count = len(G.nodes())
    edge_count = len(G.edges())

    # Ajustement dynamique de la taille de la figure en fonction du nombre de nœuds
    plt.figure(figsize=(min(20, 2 + node_count), min(20, 2 + node_count)))

    # Ajustement de la mise en page dynamique en fonction du nombre de nœuds
    pos = nx.spring_layout(G, k=min(2, 10 / node_count))  # k controls spacing between nodes

    # Ajustement de la taille des nœuds et de la police en fonction du nombre de nœuds
    node_size = max(3000 - (node_count * 50), 100)
    font_size = max(12 - (node_count * 0.2), 6)

    # Dessiner les nœuds avec les étiquettes
    nx.draw(G, pos, with_labels=True, node_size=node_size, node_color="lightblue", font_size=font_size,
            font_weight="bold", arrows=True)

    # Ajustement de la taille des étiquettes pour les types de liens en fonction du nombre d'arêtes
    edge_font_size = max(10 - (edge_count * 0.1), 6)

    # Ajouter des étiquettes pour les types de liens (les arêtes)
    edge_labels = nx.get_edge_attributes(G, 'label')
    nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels, font_color='red', font_size=edge_font_size)
